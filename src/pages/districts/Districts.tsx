...
const columns: (memberList: Member[]) => ColumnDef<DistrictDbType>[] = (memberList: Member[]) => [
  {
    accessorKey: 'view',
    header: 'View',
    cell: info => <Link to={`/viewdistrict/${info.row.original.id}`}> <Button size="icon"> <View /> </Button>
    </Link>
  },
  {
    accessorKey: 'edit',
    header: 'Edit',
    cell: info => <Link to={`/district/${info.row.original.id}`}> <Button size="icon"> <FilePenLine /> </Button> </Link>
  },
  {
    accessorKey: 'name',
  ...
  },
  {
    ...
    header: 'Leader',
    cell: info => {
      const leader = memberList?.findLast(m => m.id == info.row.original.leaderId)
  ...
  },
  {
    ...
    header: 'Assistant',
    cell: info => {
      const assistant = memberList?.findLast(m => m.id == info.row.original.assistantId)
